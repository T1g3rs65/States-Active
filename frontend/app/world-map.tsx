...
const MAP_CACHE_KEY = `world_map_terrain_v16_polecells_${WORLD_SEED}`;
const MAX_ZOOM = 8;
...
export default function WorldMap() {
  ...
  const loadMap = async () => {
    try {
      ...
      const worldCacheKey = `world_map_terrain_v16_${terrainKey(mapSeed, terrainRef.current)}`;
      ...
      await generateAndCacheTerrain(worldSeed, `world_map_terrain_v16_${terrainKey(worldSeed, terrainRef.current)}`);
    }
  };
