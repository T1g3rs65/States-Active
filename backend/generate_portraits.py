"""DEPRECATED (GH-78): standalone advisor-portrait generator.

This script previously used the `emergentintegrations` provider
(`OpenAIImageGeneration`) to generate advisor portraits. That provider is no
longer supported or installed — image generation now routes through the OpenClaw
gateway (see `grok_client.py` / `portrait_service.py`).
