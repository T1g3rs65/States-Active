"""DEPRECATED (GH-78): standalone female-marshal portrait generator.

This script previously used the `emergentintegrations` provider
(`OpenAIImageGeneration`) to generate a female Marshal portrait. That provider
is no longer supported or installed — image generation now routes through the
OpenClaw gateway (see `grok_client.py` / `portrait_service.py`).
